...
bool Adafruit_10DOF::accelGetOrientation(sensors_event_t *event, sensors_vec_t *orientation)
{
  ...
  /* heading: Rotation around the vertical axis 'Z axis'). -180<=heading<=180)                */
  /*                                                                                          */
  /*                                 z                                                        */
  /*          heading = atan(-----------------)                                               */
  /*                          sqrt(y^2 + x^2)                                                 */
  /* where:  x, y, z are returned value from accelerometer sensor                             */
  t_heading = event->acceleration.y * event->acceleration.y + event->acceleration.x * event->acceleration.x;
  orientation->heading = (float)atan2(event->acceleration.z, sqrt(t_heading)) * 180 / PI;
